...
export default function HalloWeltPage() {
  const [records, setRecords] = useState<HalloWelt[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<HalloWelt | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<HalloWelt | null>(null);
  const [viewingRecord, setViewingRecord] = useState<HalloWelt | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getHalloWelt());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: HalloWelt['fields']) {
    await LivingAppsService.createHalloWeltEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: HalloWelt['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateHalloWeltEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Hallo Welt"
      subtitle={`${records.length} Hallo Welt in the system`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={2} className="text-center py-16 text-muted-foreground">
                  {search ? 'No results found.' : 'No Hallo Welt yet. Add one!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
