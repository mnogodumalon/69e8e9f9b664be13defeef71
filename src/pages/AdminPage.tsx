...
import { HalloWeltViewDialog } from '@/components/dialogs/HalloWeltViewDialog';
...
const HALLOWELT_FIELDS = [
  { key: 'nachricht', label: 'Nachricht', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'hallo_welt', label: 'Hallo Welt', pascal: 'HalloWelt' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('hallo_welt');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'hallo_welt': return (data as any).halloWelt as HalloWelt[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'hallo_welt': return HALLOWELT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'hallo_welt': return {
        create: (fields: any) => LivingAppsService.createHalloWeltEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateHalloWeltEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteHalloWeltEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'hallo_welt' || dialogState?.entity === 'hallo_welt') && (
        <HalloWeltDialog
          open={createEntity === 'hallo_welt' || dialogState?.entity === 'hallo_welt'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'hallo_welt' ? handleUpdate : (fields: any) => handleCreate('hallo_welt', fields)}
          defaultValues={dialogState?.entity === 'hallo_welt' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['HalloWelt']}
          enablePhotoLocation={AI_PHOTO_LOCATION['HalloWelt']}
        />
      )}
      {viewState?.entity === 'hallo_welt' && (
        <HalloWeltViewDialog
          open={viewState?.entity === 'hallo_welt'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'hallo_welt', record: r }); }}
        />
      )}
