...
interface HalloWeltDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: HalloWelt['fields']) => Promise<void>;
  defaultValues?: HalloWelt['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function HalloWeltDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: HalloWeltDialogProps) {
  const [fields, setFields] = useState<Partial<HalloWelt['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'hallo_welt');
      await onSubmit(clean as HalloWelt['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "nachricht": string | null, // Nachricht\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<HalloWelt['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Edit Hallo Welt' : 'New Hallo Welt';
