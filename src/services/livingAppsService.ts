...
import type { HalloWelt, CreateHalloWelt } from '@/types/app';
...
export class LivingAppsService {
  // --- HALLO_WELT ---
  static async getHalloWelt(): Promise<HalloWelt[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HALLO_WELT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as HalloWelt[];
    return enrichLookupFields(records, 'hallo_welt');
  }
  static async getHalloWeltEntry(id: string): Promise<HalloWelt | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.HALLO_WELT}/records/${id}`);
    const record = { record_id: data.id, ...data } as HalloWelt;
    return enrichLookupFields([record], 'hallo_welt')[0];
  }
  static async createHalloWeltEntry(fields: CreateHalloWelt) {
    return callApi('POST', `/apps/${APP_IDS.HALLO_WELT}/records`, { fields: cleanFieldsForApi(fields as any, 'hallo_welt') });
  }
  static async updateHalloWeltEntry(id: string, fields: Partial<CreateHalloWelt>) {
    return callApi('PATCH', `/apps/${APP_IDS.HALLO_WELT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'hallo_welt') });
  }
  static async deleteHalloWeltEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.HALLO_WELT}/records/${id}`);
  }
